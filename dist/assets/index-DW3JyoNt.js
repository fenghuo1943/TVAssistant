...
\r
  function hideUnrelatedElements(container, video) {\r
    // 查找并隐藏容器内所有与视频无关的UI元素\r
    const selectorsToHide = [\r
      '.header',\r
      '.header-b',\r
      '.header-b-l',\r
      '.header-b-m',\r
...
\r
    selectorsToHide.forEach(selector => {\r
      const elements = container.querySelectorAll(selector);\r
      elements.forEach(el => {\r
        // 确保不隐藏包含视频的祖先元素\r
        if (!el.contains(video) && !isEssentialElement(el)) {\r
...
`,$s={id:`yangshipin`,name:`央视频直播`,matches:[`https://www.yangshipin.cn/tv/home`],capabilities:{liveMenu:!0,volumeControl:!0},defaultConfig:{volume:.6}};function ec(e){return`
${Qs}
${e}
  `}var tc=[{manifest:$s,matches:e=>$s.matches.some(t=>e.startsWith(t)),buildInitScript:e=>ec(`window.__tvAssistantPlugins.yangshipin.init(${JSON.stringify(e)});`),buildMenuDataScript:()=>ec(`window.__tvAssistantPlugins.yangshipin.waitForMenuData(20000);`),buildSelectChannelScript:e=>ec(`window.__tvAssistantPlugins.yangshipin.selectChannel(${JSON.stringify(e)});`),buildAdjustVolumeScript:e=>ec(`window.__tvAssistantPlugins.yangshipin.adjustVolume(${e});`)}];function nc(e){return tc.find(t=>t.matches(e))??null}function rc(e,t={}){let{level:n=`error`}=t;if(console.error(`[${n}] ${e}`),typeof window<`u`){let t=document.createElement(`div`);t.className=`error-toast`,t.textContent=e;let r={info:`rgba(0, 122, 255, 0.9)`,warning:`rgba(255, 149, 0, 0.9)`,error:`rgba(255, 59, 48, 0.9)`};t.style.cssText=`
      position: fixed;
      top: 20px;
      right: 20px;
      padding: 12px 24px;
      ...
      font-size: 14px;
      max-width: 400px;
    `,document.body.appendChild(t),setTimeout(()=>{t.style.animation=`slideOut 0.3s ease`,setTimeout(()=>t.remove(),300)},3e3)}}async function ic(e,t={}){let{maxRetries:n=3,retryable:r=!0,message:i}=t,a;for(let t=0;t<n;t++)try{return await e()}catch(e){if(a=e,!r||t===n-1)break;let i=1e3*(t+1);console.warn(`重试中... (${t+1}/${n})，${i}ms 后重试`),await new Promise(e=>setTimeout(e,i))}throw i&&rc(i,{...t,level:`error`}),a}var ac={GENERAL:`general`,SITE_MANAGEMENT:`site-management`,ADD_SITE:`add-site`,ADD_LOCAL_APP:`add-local-app`,WALLPAPER:`wallpaper`},oc=[{name:`TV 直播`,badge:`LIVE`,url:`https://www.yangshipin.cn/tv/home`,theme:`theme-live`,type:`website`,icon:`https://favicon.im/www.yangshipin.cn`},{name:`央视片库`,badge:`CCTV`,url:`https://tv.cctv.com/`,theme:`theme-cctv`,type:`website`,icon:`https://favicon.im/tv.cctv.com`},{name:`抖音`,badge:`DY`,url:`https://www.douyin.com/`,theme:`theme-douyin`,type:`website`,icon:`https://favicon.im/www.douyin.com`},{name:`腾讯视频`,badge:`QQ`,url:`https://v.qq.com/`,theme:`theme-tencent`,type:`website`,icon:`https://favicon.im/v.qq.com`}];function sc(){let e=B([]);return{refs:e,setRef:(t,n)=>{t?e.value[n]=t:delete e.value[n]},focus:t=>{let n=e.value[t];n instanceof HTMLElement&&(n.focus(),n.scrollIntoView({behavior:`smooth`,block:`nearest`}))},blur:t=>{let n=e.value[t];n instanceof HTMLElement&&n.blur()},getRef:t=>e.value[t]??null,getTotal:()=>e.value.length}}function cc(e=null){let t=B(e);return{ref:t,setRef:e=>{e instanceof Element?t.value=e:t.value=null},focus:()=>{t.value instanceof HTMLElement&&(t.value.focus(),t.value.scrollIntoView({behavior:`smooth`,block:`nearest`}))},blur:()=>{t.value instanceof HTMLElement&&t.value.blur()},exists:()=>t.value!==null}}var lc=`tv-assistant-host-auto-hide-cursor-style`,uc=`tv-assistant-host-cursor-hidden`;function dc(){if(document.getElementById(lc))return;let e=document.createElement(`style`);e.id=lc,e.textContent=`
    .${uc},
    .${uc} * {
      cursor: none !important;
    }
  `,document.head.appendChild(e)}function fc(e){return`
    (() => {
  ...
  `}function hc(e={}){let{hideDelay:t=3e3,immediate:n=!1}=e,r=B(n),i=B(!0),a=B(null),o=B(!1),s=null,c=null,l=!1,u=null,d=window.require?.(`robotjs`)??null;function f(e){document.documentElement.classList.toggle(uc,!e),a.value&&(a.value.style.cursor=e?``:`none`)}function p(){s&&=(clearTimeout(s),null)}function m(){c&&=(clearInterval(c),null)}async function h(e){let t=a.value;if(!(!t||!o.value))try{await t.executeJavaScript(mc(e),!0)}catch(e){console.error(`[AutoHideMouse] webview cursor visibility sync failed:`,e)}}function g(){let e=i.value;i.value=!0,f(!0),e||h(!0)}function _(){!r.value||!i.value||(i.value=!1,f(!1),h(!1))}async function v(e){let n=a.value;if(!(!n||!o.value))try{e?await n.executeJavaScript(fc(t),!0):await n.executeJavaScript(pc(),!0)}catch(e){console.error(`[AutoHideMouse] webview cursor sync failed:`,e)}}function y(){r.value&&(i.value||g(),p(),s=setTimeout(()=>{_()},t))}function b(){y()}function x(){b()}function S(){m(),d&&(u=d.getMousePos(),c=setInterval(()=>{if(!r.value)return;let e=d.getMousePos();(!u||e.x!==u.x||e.y!==u.y)&&(u=e,b())},120))}function C(){if(r.value){y(),v(!0);return}r.value=!0}function w(){if(!r.value){g(),v(!1);return}r.value=!1,u=null}function T(){if(r.value){w();return}C()}function E(e){a.value=e,o.value=!1}function ee(){o.value=!0,r.value?(v(!0),h(i.value)):v(!1)}return Xn(()=>{l=!0,dc(),f(!0),r.value&&(document.addEventListener(`mousemove`,x,{passive:!0}),S(),y())}),Dn(r,e=>{if(l){if(e){document.addEventListener(`mousemove`,x,{passive:!0}),S(),y(),v(!0),h(i.value);return}document.removeEventListener(`mousemove`,x),p(),m(),u=null,g(),v(!1)}}),$n(()=>{document.removeEventListener(`mousemove`,x),p(),m(),u=null,g(),v(!1)}),{isEnabled:r,isMouseVisible:i,enable:C,disable:w,toggle:T,showMouse:g,hideMouse:_,resetHideTimer:y,attachWebview:E,markWebviewReady:ee}}function gc(){let e=At({visible:!1,groupIndex:0,column:`group`,groups:[{label:`央视频道`,items:[`内容稍后添加`]},{label:`卫视频道`,items:[`内容稍后添加`]}],itemIndices:[0,0]}),t=B(``),n=B(``),r=B({}),i=Q(()=>e.groups[e.groupIndex]),a=Q(()=>e.itemIndices[e.groupIndex]??0),o=Q(()=>i.value?.items??[]),s=Q(()=>t.value||i.value?.label||``);function c(){e.visible=!e.visible,e.visible||(e.column=`group`)}function l(){e.visible=!1,e.column=`group`}function u(t){let n=e.groups.length;e.groupIndex=(e.groupIndex+t+n)%n}function d(t){let n=o.value.length,r=(a.value+t+n)%n;e.itemIndices[e.groupIndex]=r}function f(n){if(!n)return;let r=n.trim();t.value=r;let i=e.groups.findIndex(e=>e.items.includes(r));if(i===-1)return;let a=e.groups[i].items.indexOf(r);e.groupIndex=i,e.itemIndices[i]=a}function p(t){let n=t.央视频道?.length?t.央视频道:[`内容稍后添加`],r=t.卫视频道?.length?t.卫视频道:[`内容稍后添加`];e.groups=[{label:`央视频道`,items:n},{label:`卫视频道`,items:r}],e.itemIndices=e.groups.map(()=>0),e.groupIndex=0,f(t.currentChannel??``)}function m(){e.column=e.column===`group`?`item`:`group`}return{state:e,currentChannel:t,currentPluginId:n,currentPluginConfig:r,currentGroup:i,currentItemIndex:a,currentItems:o,heading:s,toggle:c,close:l,moveGroup:u,moveItem:d,syncChannel:f,applyGroups:p,switchColumn:m}}var _c=function(e){return e.TOOLBAR=`toolbar`,e.CARDS=`cards`,e}(_c||{});po(Do(Fn({__name:`HomePage`,setup(e){let t=window.require?.(`electron`)?.ipcRenderer??null,n=gc(),{enable:r,disable:i,attachWebview:a,markWebviewReady:o,isMouseVisible:s}=hc({hideDelay:3e3,immediate:!1}),c=sc(),l=cc(),u=cc(),d=B(null),f=B(_c.CARDS),p=At({now:new Date,selectedIndex:0,activeUrl:``,activeTitle:``,showSettings:!1,activeSettingsMenu:ac.GENERAL,settings:{...Xo}}),m=At({currentPluginId:``,currentPluginConfig:{}}),h,g=0,_=new Intl.DateTimeFormat(`zh-CN`,{month:`long`,day:`numeric`,weekday:`long`}),v=Q(()=>[...oc,...p.settings.customShortcuts].filter(e=>p.settings.enabledShortcuts.includes(e.url))),y=Q(()=>p.now.toLocaleTimeString(`zh-CN`,{hour:`2-digit`,minute:`2-digit`,hour12:!1})),b=Q(()=>nc(p.activeUrl)),x=Q(()=>_.format(p.now)),S=Q(()=>b.value?.manifest.capabilities.liveMenu??!1);function C(){p.now=new Date}function w(){p.showSettings=!0,i()}function T(){p.showSettings=!1,H(()=>{k()})}function E(e){if(e.type===`application`){ee(e);return}p.activeUrl=e.url,p.activeTitle=e.name,p.showSettings=!1,n.close(),n.currentChannel.value=``,m.currentPluginId=``,m.currentPluginConfig={},r(),H(()=>{let e=u.ref.value;e?e.focus():l.ref.value?.focus()})}async function ee(e){try{console.log(`正在打开本地应用: ${e.name}`,e.url);let n=await t?.invoke(`app:open-local`,e.url);n?.success?console.log(`成功打开应用: ${e.name}`):(console.error(`打开应用失败: ${n?.error}`),alert(`无法打开应用: ${n?.error||`未知错误`}`))}catch(e){console.error(`打开本地应用时出错:`,e),alert(`打开应用时出错: ${e instanceof Error?e.message:`未知错误`}`)}}function D(){if(!p.settings.openModuleOnLaunch||!p.settings.launchModuleId)return;let e=oc.find(e=>e.url===p.settings.launchModuleId);e&&E(e)}function te(){p.activeUrl=``,p.activeTitle=``,n.close(),n.currentChannel.value=``,m.currentPluginId=``,m.currentPluginConfig={},i(),H(()=>{k()})}function O(e,t){c.setRef(e,t)}function ne(e){l.setRef(e)}function re(e){u.setRef(e),a(e??null)}function k(){c.getRef(p.selectedIndex)?.focus(),f.value=_c.CARDS}function ie(){console.log(`[HomePage] focusToolbar 被调用`),console.log(`[HomePage] homeLandingRef:`,d.value),d.value&&(d.value.focusToolbar(),f.value=_c.TOOLBAR,console.log(`[HomePage] 焦点区域已设置为 TOOLBAR`))}function A(e=!1){e&&(p.selectedIndex=0),k()}function j(){let e=window.innerWidth;return e<=640?3:e<=1100?4:5}function ae(e){let t=v.value.length;if(t===0)return;let n=j(),r=p.selectedIndex;switch(e){case`left`:if(r%n===0){let e=Math.floor(r/n),i=e===0?Math.ceil(t/n)-1:e-1,a=Math.min(n-1,t-1-i*n);r=i*n+a}else --r;break;case`right`:(r+1)%n===0||r===t-1?r=(Math.floor(r/n)+1)%Math.ceil(t/n)*n:r+=1;break;case`up`:{let e=Math.floor(r/n),i=r%n,a=Math.ceil(t/n);if(e===0){let e=(a-1)*n,o=Math.min(t-1,e+n-1);r=e+Math.min(i,o-e)}else r=(e-1)*n+i}break;case`down`:{let e=Math.floor(r/n),i=r%n;e===Math.ceil(t/n)-1?(r=i,r>=t&&(r=t-1)):(r=(e+1)*n+i,r>=t&&(r=t-1))}break}r=Math.max(0,Math.min(t-1,r)),p.selectedIndex=r,k()}function oe(e){console.log(`[HomePage] handleToolbarNavigation 被调用，direction:`,e),d.value?(console.log(`[HomePage] 调用子组件的 moveToolbarFocus`),d.value.moveToolbarFocus(e)):console.warn(`[HomePage] homeLandingRef 为空`)}function se(){S.value&&n.toggle()}function ce(){n.close()}function le(e){let t=n.state.groupIndex;n.moveGroup(e),t!==n.state.groupIndex&&H(()=>{setTimeout(()=>{let e=document.querySelector(`.live-menu-channel-item.is-active`)||document.querySelector(`.live-menu-channel-item.is-selected`);e&&e.scrollIntoView({behavior:`smooth`,block:`nearest`})},80)})}function ue(e){n.moveItem(e),H(()=>{setTimeout(()=>{let e=document.querySelector(`.live-menu-channel-item.is-active`)||document.querySelector(`.live-menu-channel-item.is-selected`);e&&e.scrollIntoView({behavior:`smooth`,block:`nearest`})},50)})}function de(e){n.syncChannel(e)}function fe(e){n.applyGroups(e)}async function pe(e){try{return await ic(async()=>await t?.invoke(`plugin-config:get`,e)??{},{message:`加载插件 ${e} 配置失败`,retryable:!0,maxRetries:3})}catch{return rc(`无法加载插件配置，请检查网络连接`),{}}}async function me(e,n){try{m.currentPluginConfig=n,await ic(async()=>{await t?.invoke(`plugin-config:set`,e,n)},{message:`保存插件 ${e} 配置失败`,retryable:!0,maxRetries:3})}catch(e){throw rc(`保存插件配置失败，请稍后重试`),e}}async function he(){try{let e=await ic(async()=>await t?.invoke(`settings:get`),{message:`加载设置失败`,retryable:!0,maxRetries:3});p.settings={...Xo,...e}}catch{rc(`无法加载设置，将使用默认配置`),p.settings={...Xo}}}async function ge(e){try{let n=JSON.parse(JSON.stringify(e));await ic(async()=>await t?.invoke(`settings:set`,n),{message:`保存设置失败`,retryable:!0,maxRetries:3}),p.settings={...p.settings,...e},rc(`设置已保存`,{level:`info`})}catch(e){throw rc(`保存设置失败，请稍后重试`),console.error(`保存设置失败:`,e),e}}async function _e(){let e=u.ref.value,t=b.value;if(!e||!t)return null;m.currentPluginId!==t.manifest.id&&(m.currentPluginConfig={...t.manifest.defaultConfig,...await pe(t.manifest.id)},m.currentPluginId=t.manifest.id);try{await e.executeJavaScript(t.buildInitScript(m.currentPluginConfig),!0)}catch(e){return console.error(`初始化插件 ${t.manifest.id} 失败:`,e),null}return t}async function N(){let e=u.ref.value,t=b.value;if(!e||!t?.manifest.capabilities.liveMenu)return;let n=++g;try{await _e();let r=await e.executeJavaScript(t.buildMenuDataScript(),!0);if(n!==g||!r)return;fe(r)}catch(e){console.error(`获取插件 ${t.manifest.id} 菜单数据失败:`,e),fe({})}}async function ve(e){let t=u.ref.value,n=b.value;if(!(!t||!n?.manifest.capabilities.liveMenu))try{de(e),await _e(),await t.executeJavaScript(n.buildSelectChannelScript(e),!0)&&(ce(),window.setTimeout(()=>{N()},1200))}catch(e){console.error(`插件 ${n.manifest.id} 切换频道失败:`,e)}}async function ye(e){let t=u.ref.value,n=b.value;if(!(!t||!n?.manifest.capabilities.volumeControl))try{await _e();let r=await t.executeJavaScript(n.buildAdjustVolumeScript(e),!0);typeof r==`number`&&await me(n.manifest.id,{...m.currentPluginConfig,volume:r})}catch(e){console.error(`插件 ${n.manifest.id} 调整音量失败:`,e)}}function P(){o();let e=b.value;e&&(_e(),e.manifest.capabilities.liveMenu&&N())}function be(e){p.showSettings&&p.activeUrl}function xe(e){if(console.log(`[HomePage] 按键事件:`,e.key),!(p.showSettings&&!p.activeUrl)){if(p.activeUrl){if(console.log(`[HomePage] 当前活动 URL:`,p.activeUrl),n.state.visible){if(e.key===$.BACK){e.preventDefault(),ce();return}if(e.key===$.LEFT||e.key===$.RIGHT){e.preventDefault(),n.switchColumn();return}if(e.key===$.UP){if(e.preventDefault(),n.state.column===`group`){le(-1);return}ue(-1);return}if(e.key===$.DOWN){if(e.preventDefault(),n.state.column===`group`){le(1);return}ue(1);return}if(e.key===$.CONFIRM&&n.state.column===`item`){e.preventDefault(),ve(n.currentItems.value[n.currentItemIndex.value]);return}return}if(S.value&&e.key===$.MENU){e.preventDefault(),console.log(`[HomePage] 菜单键，切换直播菜单`),se();return}if(os(e.key)){e.preventDefault(),ye(e.key===$.MINUS||e.key===$.UNDERSCORE?-.05:.05);return}e.key===$.BACK&&(e.preventDefault(),te());return}if(f.value===_c.TOOLBAR){if(e.key===$.LEFT||e.key===$.RIGHT){e.preventDefault(),oe(e.key===$.LEFT?`left`:`right`);return}if(e.key===$.DOWN){e.preventDefault(),A(!0);return}if(e.key===$.CONFIRM){if(e.preventDefault(),console.log(`[HomePage] 工具栏确认键，触发按钮点击`),d.value){let e=d.value;console.log(`[HomePage] landingInstance:`,e.currentToolbarIndex);let t=e.toolbarButtonRefs,n=e.currentToolbarIndex;if(console.log(`[HomePage] buttons:`,t,`currentIndex:`,n),t&&t.length>0&&typeof n==`number`){let e=t[n];console.log(`[HomePage] 准备点击的按钮:`,e),e&&(e.click(),console.log(`[HomePage] 按钮已点击`))}else console.log(`[HomePage] buttons:`,t,`currentIndex:`,n),console.warn(`[HomePage] 按钮数组为空或 currentIndex 不是数字`),console.warn(`[HomePage] landingInstance:`,e)}else console.warn(`[HomePage] homeLandingRef 为空`);return}}else{if(e.key===$.UP){e.preventDefault();let t=j();if(Math.floor(p.selectedIndex/t)===0){ie();return}ae(`up`);return}if(e.key===$.DOWN){e.preventDefault(),ae(`down`);return}if(e.key===$.LEFT){e.preventDefault(),ae(`left`);return}if(e.key===$.RIGHT){e.preventDefault(),ae(`right`);return}e.key===$.CONFIRM&&(e.preventDefault(),v.value.length>0&&E(v.value[p.selectedIndex]))}}}function F(e,t){xe(new KeyboardEvent(`keydown`,{key:t.key}))}function Se(){window.close()}return Xn(async()=>{C(),h=window.setInterval(C,1e3),t?.on(`app-keydown`,F),await he(),D(),H(()=>{k()})}),$n(()=>{h&&window.clearInterval(h),i(),t?.removeListener(`app-keydown`,F)}),(e,t)=>(J(),Y(`main`,{class:M([`home-shell`,{"is-browser-open":!!p.activeUrl}]),tabindex:`0`,onKeydown:xe,onKeydownCapture:be},[p.showSettings?(J(),Ai(Zs,{key:0,"active-menu":p.activeSettingsMenu,settings:p.settings,onBack:T,onSelectMenu:t[0]||=e=>p.activeSettingsMenu=e,onUpdateSetting:ge},null,8,[`active-menu`,`settings`])):p.activeUrl?(J(),Ai(Oo,{key:2,"active-url":p.activeUrl,"set-back-button-ref":ne,"set-webview-ref":re,"show-live-menu":Ht(n).state.visible,"live-menu-groups":Ht(n).state.groups,"active-live-group-index":Ht(n).state.groupIndex,"active-live-column":Ht(n).state.column,"active-live-item-index":Ht(n).currentItemIndex.value,"live-menu-heading":Ht(n).heading.value,"is-mouse-visible":Ht(s),onBrowserReady:P,onGoHome:te,onSelectLiveChannel:ve},null,8,[`active-url`,`show-live-menu`,`live-menu-groups`,`active-live-group-index`,`active-live-column`,`active-live-item-index`,`live-menu-heading`,`is-mouse-visible`])):(J(),Ai(Wo,{key:1,ref_key:`homeLandingRef`,ref:d,"current-time":y.value,"current-date":x.value,shortcuts:v.value,"selected-index":p.selectedIndex,"set-card-ref":O,onOpenSettings:w,onCloseWindow:Se,onOpenSite:E,onFocusCard:t[1]||=e=>p.selectedIndex=e,onToolbarFocusChange:t[2]||=e=>f.value=e?_c.TOOLBAR:_c.CARDS},null,8,[`current-time`,`current-date`,`shortcuts`,`selected-index`]))],34))}}),[[`__scopeId`,`data-v-9df13b65`]])).mount(`#app`);