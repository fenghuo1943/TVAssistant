...
`,Xo={id:`yangshipin`,name:`央视频直播`,matches:[`https://www.yangshipin.cn/tv/home`],capabilities:{liveMenu:!0,volumeControl:!0},defaultConfig:{volume:.6}};function Zo(e){return`
${Yo}
${e}
  `}var Qo=[{manifest:Xo,matches:e=>Xo.matches.some(t=>e.startsWith(t)),buildInitScript:e=>Zo(`window.__tvAssistantPlugins.yangshipin.init(${JSON.stringify(e)});`),buildMenuDataScript:()=>Zo(`window.__tvAssistantPlugins.yangshipin.waitForMenuData(20000);`),buildSelectChannelScript:e=>Zo(`window.__tvAssistantPlugins.yangshipin.selectChannel(${JSON.stringify(e)});`),buildAdjustVolumeScript:e=>Zo(`window.__tvAssistantPlugins.yangshipin.adjustVolume(${e});`)}];function $o(e){return Qo.find(t=>t.matches(e))??null}$a(po(Mn({__name:`HomePage`,setup(e){let t=window.require?.(`electron`)?.ipcRenderer??null,n=V(new Date),r=V(0),i=V(``),a=V(``),o=V(!1),s=V(`general`),c=V({...ko}),l=V([]),u=V(null),d=V(null),f=V(!1),p=V(0),m=V(`group`),h=V([0,0]),g=V(``),_=V(``),v=V({}),y=V([{label:`央视频道`,items:[`内容稍后添加`]},{label:`卫视频道`,items:[`内容稍后添加`]}]),b,x=0,S=new Intl.DateTimeFormat(`zh-CN`,{month:`long`,day:`numeric`,weekday:`long`}),C=ua(()=>n.value.toLocaleTimeString(`zh-CN`,{hour:`2-digit`,minute:`2-digit`,hour12:!1})),w=ua(()=>$o(i.value)),ee=ua(()=>S.format(n.value)),te=ua(()=>w.value?.manifest.capabilities.liveMenu??!1),ne=ua(()=>y.value[p.value]?.items??[]),T=ua(()=>h.value[p.value]??0),re=ua(()=>g.value||y.value[p.value]?.label||``);function E(){n.value=new Date}function ie(){o.value=!0}function ae(){o.value=!1,sn(()=>{ue()})}function D(e){i.value=e.url,a.value=e.name,o.value=!1,pe(),g.value=``,_.value=``,v.value={},sn(()=>{u.value?.focus()})}function oe(){if(!c.value.openModuleOnLaunch||!c.value.launchModuleId)return;let e=Oo.find(e=>e.url===c.value.launchModuleId);e&&D(e)}function O(){i.value=``,a.value=``,pe(),g.value=``,_.value=``,v.value={},sn(()=>{ue()})}function se(e,t){if(e instanceof HTMLButtonElement){l.value[t]=e;return}delete l.value[t]}function ce(e){u.value=e instanceof HTMLButtonElement?e:null}function le(e){d.value=e}function ue(){l.value[r.value]?.focus()}function de(e){let t=Oo.length;r.value=(r.value+e+t)%t,ue()}function fe(){te.value&&(f.value=!f.value,f.value||(m.value=`group`))}function pe(){f.value=!1,m.value=`group`}function me(e){let t=y.value.length;p.value=(p.value+e+t)%t}function he(e){let t=ne.value.length,n=(T.value+e+t)%t;h.value[p.value]=n}function ge(e){if(!e)return;let t=e.trim();g.value=t;let n=y.value.findIndex(e=>e.items.includes(t));if(n===-1)return;let r=y.value[n].items.indexOf(t);p.value=n,h.value[n]=r}function _e(e){let t=e.央视频道?.length?e.央视频道:[`内容稍后添加`],n=e.卫视频道?.length?e.卫视频道:[`内容稍后添加`];y.value=[{label:`央视频道`,items:t},{label:`卫视频道`,items:n}],h.value=y.value.map(()=>0),p.value=0,ge(e.currentChannel??``)}async function ve(e){return await t?.invoke(`plugin-config:get`,e)??{}}async function ye(e,n){v.value=n,await t?.invoke(`plugin-config:set`,e,n)}async function be(){let e=await t?.invoke(`settings:get`);c.value={...ko,...e??{}}}async function A(e){let n=await t?.invoke(`settings:set`,e);c.value={...c.value,...e,...n??{}}}async function j(){let e=d.value,t=w.value;if(!e||!t)return null;_.value!==t.manifest.id&&(v.value={...t.manifest.defaultConfig,...await ve(t.manifest.id)},_.value=t.manifest.id);try{await e.executeJavaScript(t.buildInitScript(v.value),!0)}catch(e){return console.error(`初始化插件 ${t.manifest.id} 失败:`,e),null}return t}async function xe(){let e=d.value,t=w.value;if(!e||!t?.manifest.capabilities.liveMenu)return;let n=++x;try{await j();let r=await e.executeJavaScript(t.buildMenuDataScript(),!0);if(n!==x||!r)return;_e(r)}catch(e){console.error(`获取插件 ${t.manifest.id} 菜单数据失败:`,e),_e({})}}async function M(e){let t=d.value,n=w.value;if(!(!t||!n?.manifest.capabilities.liveMenu))try{ge(e),await j(),await t.executeJavaScript(n.buildSelectChannelScript(e),!0)&&(pe(),window.setTimeout(()=>{xe()},1200))}catch(e){console.error(`插件 ${n.manifest.id} 切换频道失败:`,e)}}async function Se(e){let t=d.value,n=w.value;if(!(!t||!n?.manifest.capabilities.volumeControl))try{await j();let r=await t.executeJavaScript(n.buildAdjustVolumeScript(e),!0);typeof r==`number`&&await ye(n.manifest.id,{...v.value,volume:r})}catch(e){console.error(`插件 ${n.manifest.id} 调整音量失败:`,e)}}function Ce(){let e=w.value;e&&(j(),e.manifest.capabilities.liveMenu&&xe())}function N(e){if(o.value&&!i.value&&(e.key===`Escape`||e.key===`Backspace`)){e.preventDefault(),e.stopPropagation(),ae();return}}function we(e){if(o.value&&!i.value){console.log(e.key),(e.key===`Escape`||e.key===`Backspace`)&&(e.preventDefault(),ae());return}if(i.value){if(f.value){if(e.key===`Escape`||e.key===`Backspace`){e.preventDefault(),pe();return}if(e.key===`ArrowLeft`||e.key===`ArrowRight`){e.preventDefault(),m.value=m.value===`group`?`item`:`group`;return}if(e.key===`ArrowUp`){if(e.preventDefault(),m.value===`group`){me(-1);return}he(-1);return}if(e.key===`ArrowDown`){if(e.preventDefault(),m.value===`group`){me(1);return}he(1);return}if(e.key===`Enter`&&m.value===`item`){e.preventDefault(),M(ne.value[T.value]);return}return}if(te.value&&e.key===`Enter`){e.preventDefault(),fe();return}if(e.key===`-`||e.key===`_`){e.preventDefault(),Se(-.05);return}if(e.key===`=`||e.key===`+`){e.preventDefault(),Se(.05);return}(e.key===`Escape`||e.key===`Backspace`)&&(e.preventDefault(),O());return}if(e.key===`ArrowLeft`||e.key===`ArrowUp`){e.preventDefault(),de(-1);return}if(e.key===`ArrowRight`||e.key===`ArrowDown`){e.preventDefault(),de(1);return}(e.key===`Enter`||e.key===` `)&&(e.preventDefault(),D(Oo[r.value]))}function Te(e,t){we(new KeyboardEvent(`keydown`,{key:t.key}))}function Ee(){window.close()}return qn(async()=>{E(),b=window.setInterval(E,1e3),t?.on(`app-keydown`,Te),await be(),oe(),sn(()=>{ue()})}),Xn(()=>{b&&window.clearInterval(b),t?.removeListener(`app-keydown`,Te)}),(e,t)=>(X(),Z(`main`,{class:k([`home-shell`,{"is-browser-open":!!i.value}]),tabindex:`0`,onKeydown:we,onKeydownCapture:N},[o.value?(X(),Di(Jo,{key:0,"active-menu":s.value,settings:c.value,onBack:ae,onSelectMenu:t[0]||=e=>s.value=e,onUpdateSetting:A},null,8,[`active-menu`,`settings`])):i.value?(X(),Di(mo,{key:2,"active-url":i.value,"set-back-button-ref":ce,"set-webview-ref":le,"show-live-menu":f.value,"live-menu-groups":y.value,"active-live-group-index":p.value,"active-live-column":m.value,"active-live-item-index":T.value,"live-menu-heading":re.value,onBrowserReady:Ce,onGoHome:O,onSelectLiveChannel:M},null,8,[`active-url`,`show-live-menu`,`live-menu-groups`,`active-live-group-index`,`active-live-column`,`active-live-item-index`,`live-menu-heading`])):(X(),Di(Do,{key:1,"current-time":C.value,"current-date":ee.value,shortcuts:Vt(Oo),"selected-index":r.value,"set-card-ref":se,onOpenSettings:ie,onCloseWindow:Ee,onOpenSite:D,onFocusCard:t[1]||=e=>r.value=e},null,8,[`current-time`,`current-date`,`shortcuts`,`selected-index`]))],34))}}),[[`__scopeId`,`data-v-d06a42ef`]])).mount(`#app`);